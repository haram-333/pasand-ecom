...
// Use the actual kid product interface from the data
...
interface KidFilterProps {
    onFilterChange?: (filteredProducts: KidProduct[]) => void;
}

// Use the actual kid products data instead of hardcoded data
const allKidProducts: KidProduct[] = kidProducts;

export default function KidFilter({ onFilterChange }: KidFilterProps) {
    ...
    // Calculate availability options based on actual kid data
    const availabilityOptions = [
        { value: 'in-stock', label: 'In Stock', count: allKidProducts.filter(p => p.availability === 'in-stock').length },
        { value: 'out-of-stock', label: 'Out of Stock', count: allKidProducts.filter(p => p.availability === 'out-of-stock').length }
    ];

    // Kids specific sizes
    const sizeOptions = [
        { value: '5-6', label: '5-6', count: allKidProducts.filter(p => p.sizes.includes('5-6') && p.sizeStock['5-6']?.available).length },
        { value: '7-8', label: '7-8', count: allKidProducts.filter(p => p.sizes.includes('7-8') && p.sizeStock['7-8']?.available).length },
        { value: '9-10', label: '9-10', count: allKidProducts.filter(p => p.sizes.includes('9-10') && p.sizeStock['9-10']?.available).length },
        { value: '11-12', label: '11-12', count: allKidProducts.filter(p => p.sizes.includes('11-12') && p.sizeStock['11-12']?.available).length },
        { value: '13-14', label: '13-14', count: allKidProducts.filter(p => p.sizes.includes('13-14') && p.sizeStock['13-14']?.available).length }
    ];
    ...
    // Filtering logic for kid products
    const filteredProducts = useMemo(() => {
        let result = allKidProducts;
        ...
        return result;
    }, [filters, allKidProducts]);

    const totalProducts = allKidProducts.length;
    ...
    return (
        <div className="w-full bg-white p-4 md:p-6 mx-auto max-w-7xl">
            <h2 className="text-gray-900 text-lg md:text-xl font-bold mb-4">Filter Kids Products</h2>
                                ...
                                <NavigationMenuItem>
                                    ...
                                    <NavigationMenuContent>
                                        <div className="w-44 p-3 bg-white rounded-lg shadow-lg border overflow-hidden">
                                            <h3 className="font-bold text-gray-900 mb-3 text-sm">Kids Size</h3>
                                            <div className="space-y-2">
